...
enum AnimationDirection { slideDown, slideUp }
...
class SignupField extends StatefulWidget {
  ...
  const SignupField({
    super.key,
    required this.child,
    this.direction = AnimationDirection.slideDown,
  });
...
class _SignupFieldState extends State<SignupField> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // Configure slide animation based on the direction
    slideAnimation = Tween<double>(
      begin: widget.direction == AnimationDirection.slideDown ? -50.0 : 50.0,
      end: 0.0,
    ).animate(
      CurvedAnimation(
        parent: controller,
