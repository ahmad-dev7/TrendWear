        ...
        body: MultiBlocProvider(
          ...
          child: BlocConsumer<JwtTokenCubit, JwtTokenState>(
            ...
            builder: (context, state) {
              if (state is JwtTokenInitial) {
                context.read<JwtTokenCubit>().checkJwtToken();
              }
              if (state is JwtTokenExpired || state is JwtTokenNotCreated) {
                return const LoginSignupScreen();
              }
              if (state is JwtTokenValid) {
                return const HomeScreen();
              }
              return "building".text.makeCentered();
            },
          ),
        ),
      ),
